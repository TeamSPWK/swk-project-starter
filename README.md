# swk-project-standard-starter

SPACEWALK 의 **웹 풀스택 표준 스타터** — Next.js(App Router) 프로젝트를 에이전틱 개발 규율까지 갖춘 상태로 시작하는 GitHub 템플릿입니다.
...
> 🔴 **이 템플릿으로 만든 프로젝트는 MIT 가 아닙니다.** "Use this template" 은 파일을 복사하므로 `LICENSE` 도 따라갑니다 — `setup.sh` 가 이를 제거하고, 새 프로젝트는 기본적으로 **사내 비공개(proprietary)** 로 시작합니다. 공개할 프로젝트라면 라이선스를 **명시적으로 다시 선택**하세요.
