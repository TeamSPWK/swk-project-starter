# swk-project-starter — 템플릿 레포 자체를 고칠 때
...
삭제한다 — 안 그러면 사내 비공개 프로젝트가 MIT 를 달고 시작한다. `README.md` 는 `README.md.template`
이 덮어쓴다. 이 파일도 같은 방식(2번)으로 처리돼 있다.

⚠️ §2.5 의 삭제 가드는 `LICENSE` 안의 `MIT License` **와** `Copyright (c) 2026 SPACEWALK` 두 문자열을
모두 확인한다. 연도나 주체를 고치면 **조용히 삭제가 멈추고** MIT 가 새 프로젝트로 그대로 실린다 —
`LICENSE` 를 손대면 그 가드도 같이 고친다.

⚠️ **`CLAUDE.md` 를 symlink 로 바꾸지 말 것.** 회사 표준은 `@AGENTS.md` import 와 symlink 를 **둘 다**
허용하지만(swk-wiki `40_Playbooks/_System/agent-rules/README.md`), 이 레포에서는 symlink 가 `setup.sh` 를
